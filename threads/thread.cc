...
#define MAX_PRODUCE_NUM 65536
#define N_PROD 256
#define N_CONS 256
#define RING_LENGTH 32

Semaphore *mutex = NULL;
Semaphore *not_empty = NULL;
...
static void Consumer(void *arg) {
	while (true) {
		...
		mutex->P(); {
			...
			struct buf_node *node = buf_head->next;
			buf_head->next = buf_head->next->next;
			if (node == buf_tail) buf_tail = buf_head;
			ret = node->value;
			delete node;
			buf_count--;
...
Thread::SelfTest()
{
	...
	not_empty = new Semaphore("not empty", 0);
	buf_head = new buf_node;
	buf_head->next = NULL;
	buf_tail = buf_head;
	...
	for (size_t i = 0; i < N_CONS; i++) {
		std::cout << "Spawning consumer thread " << i << "\n";
		char buf[64];
	...
	for (size_t i = 0; i < N_PROD; i++) {
		std::cout << "Spawning producer thread " << i << "\n";
		char buf[64];
	...
	while (true) {
		if (count >= MAX_PRODUCE_NUM && get_buf_count() == 0)
			return;
